<x-app-layout>
            ...
            <ol class="inline-flex items-center space-x-1 md:space-x-3">
                ...
                <li>
                    <div class="flex items-center">
                        ...
                        <a href="{{ route('staff.index') }}" class="ml-1 text-sm font-medium text-gray-700 hover:text-blue-600 md:ml-2 dark:text-gray-400 dark:hover:text-white">{{ __('Staff') }}</a>
                    </div>
                </li>
                <li aria-current="page">
                <div class="flex items-center">
                    <svg aria-hidden="true" class="w-6 h-6 text-gray-400" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z" clip-rule="evenodd"></path></svg>
                    <span class="ml-1 text-sm font-medium text-gray-500 md:ml-2 dark:text-gray-400">{{__('Update')}}</span>
                </div>
                </li>
            </ol>
        </nav>
    ...
    <div class="bg-white shadow-md dark:bg-gray-800 rounded-lg p-6">
        <h3 class="mb-6 capitalize text-2xl font-semibold text-gray-700 dark:text-gray-200"> Update Staff </h3>
        <!-- General elements -->
        <form method="POST" action="{{ route('staff.update', $staff->id) }}">
          ...
          <div class="py-3 grid gap-4 lg:grid-cols-2">
            <div class="block text-sm">
              ...
              <x-text-input id="phone" class="block mt-1 w-full" type="text" name="phone" :value="old('phone', $staff->phone)" required autofocus />
            ...
            <div class="block text-sm">
              ...
              <x-text-input id="zip_code" class="block mt-1 w-full" type="text" name="zip_code" placeholder="2000" :value="old('zip_code', $staff->zip_code)" required autofocus />
              <x-input-error :messages="$errors->get('zip_code')" class="mt-2" />
            </div>
            <div class="block text-sm">
              <x-input-label for="address" :value="__('Address')" />
              <x-text-input id="address" class="block mt-1 w-full" type="text" name="address" placeholder="Address 1, address 2" :value="old('address', $staff->address)" required autofocus />
            ...
            <div class="block text-sm">
              ...
              <select id="ngo" name="ngo_id" class="block w-full mt-1 text-sm dark:text-gray-300 dark:border-gray-600 dark:bg-gray-700 form-select focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:focus:shadow-outline-gray">
                @foreach($ngos as $ngo)
                  <option value="{{$ngo->id}}" {{$staff->ngo_id == $ngo->id ? 'selected' : ''}}>{{$ngo->name}}</option>
                @endforeach
              </select>
            ...
            <div class="block text-sm">
              ...
              <select id="designation_id" name="designation_id" class="block w-full mt-1 text-sm dark:text-gray-300 dark:border-gray-600 dark:bg-gray-700 form-select focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:focus:shadow-outline-gray">
                  ...
                  <option value="{{$designation->id}}" {{$staff->designation_id == $designation->id ? 'selected' : ''}}>{{$designation->name}}</option>
                  @endforeach
              </select>
            ...
            <div class="block text-sm">
              ...
              <select id="user_id" name="user_id" class="block w-full mt-1 text-sm dark:text-gray-300 dark:border-gray-600 dark:bg-gray-700 form-select focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:focus:shadow-outline-gray">
                  @foreach($users as $user)
                    <option value="{{$user->id}}" {{$staff->user_id == $user->id ? 'selected' : ''}}>{{$user->name}}</option>
                  @endforeach
              </select>
            ...
            <div class="block text-sm">
              ...
              <x-text-input id="facebook_url" class="block mt-1 w-full" type="text" name="facebook_url" placeholder="https://www.facebook.com/xxxx" :value="old('facebook_url', $staff->facebook_url)" required autofocus />
              <x-input-error :messages="$errors->get('facebook_url')" class="mt-2" />
            </div>
            <div class="block text-sm">
              <x-input-label for="whatsapp_number" :value="__('WhatsApp number')" />
              <x-text-input id="whatsapp_number" class="block mt-1 w-full" type="text" name="whatsapp_number" placeholder="xxxxxxxx" :value="old('whatsapp_number', $staff->whatsapp_number)" required autofocus />
